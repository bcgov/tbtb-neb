            ...
            <div class="card-header">
                <div>Bursary Period
                    <template v-if="periodStats !== ''">
                        <span class="badge bg-primary fw-light">{{ periodStats.info.bpsd }}</span> to <span
                            class="badge bg-primary fw-light">{{ periodStats.info.bped }}</span>
                    </template>
                            ...
                            <div class="table-responsive">
                                ...
                                <nav aria-label="Eligibility List Pagination">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item"
                                            :class="eligibilityList.meta.current_page === 1 ?? 'disabled'">
                                            <a class="page-link"
                                                @click.prevent="jumpToPage(eligibilityList.meta.first_page)">First</a>
                                        </li>
                                        <li class="page-item"
                                            :class="eligibilityList.meta.current_page === 1 ?? 'disabled'">
                                            <a class="page-link"
                                                @click.prevent="jumpToPage(eligibilityList.meta.current_page - 1)">Previous</a>
                                        </li>
                                        <li class="page-item"><span class="page-link">{{ eligibilityList.meta.current_page
                                        }}</span></li>
                                        <li class="page-item"
                                            :class="eligibilityList.meta.current_page === eligibilityList.meta.last_page ?? 'disabled'">
                                            <a class="page-link"
                                                @click.prevent="jumpToPage(eligibilityList.meta.current_page + 1)">Next</a>
                                        </li>
                                        <li class="page-item"
                                            :class="eligibilityList.meta.current_page === eligibilityList.meta.last_page ?? 'disabled'">
                                            <a class="page-link"
                                                @click.prevent="jumpToPage(eligibilityList.meta.last_page)">Last</a>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
...
export default {
    ...
    methods: {
        ...
        exportList: function (type) {
            window.location.href = '/export-neb/' + type + '/' + this.showNebForm.bursary_period_id;
        },

        jumpToPage: function (page) {
            if (page < this.eligibilityList.meta.first_page || page > this.eligibilityList.meta.last_page) return;
        ...
        showNeb: function () {
            ...
            this.showNebForm.post('/bursary-periods/fetch-neb', {
                onSuccess: (response) => {
                    this.showNebForm.formSubmitting = false;
                    this.showNebForm.page = response.props.results.meta.current_page;
        ...
        finalizeStep: function (step) {
            if (step <= this.endNebForm.totalSteps) {
                ...
                this.endNebForm.post('/finalize-neb', {
                    onSuccess: () => {
                ...
                window.location.href = "/bursary-periods/show/" + this.showNebForm.bursary_period_id;
            }
        },
        ...
        starStep: function (step) {
            if (step <= this.startNebForm.totalSteps) {
                ...
                this.startNebForm.post('/process-neb', {
                    onSuccess: () => {
